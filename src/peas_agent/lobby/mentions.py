...
def parse_mentions(
    ...
    *,
    ...
    """Resolve @display_name tokens to agent_id list (deduped, order preserved)."""
    if not text or not display_name_to_agent_id:
        return []

    # longest display names first for greedy match
    ...
    for match in _MENTION_RE.finditer(text):
        ...
        if not raw:
            continue
        # try longest prefix match against known display names
