...
def test_join_rejects_duplicate_display_name(tmp_path: Path) -> None:
    events: list[dict] = []
...
def test_start_discussion_grants_first_speaker(tmp_path: Path) -> None:
    ...
    async def _run() -> None:
        ...
        assert granted == ["m001"]
        assert room.config.discussion_started is True
...
def test_rejoin_stale_disconnect_preserves_turn(tmp_path: Path) -> None:
    async def _run() -> None:
        room = Room(config=RoomConfig(room_id="T", discussion_started=True), workspace=tmp_path)
